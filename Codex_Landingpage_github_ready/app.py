...
def append_lead_to_google_sheet(lead: dict[str, str]) -> tuple[bool, str]:
    if "google_apps_script" in st.secrets:
        config = dict(st.secrets["google_apps_script"])
    ...
    if "google_sheet" not in st.secrets or "gcp_service_account" not in st.secrets:
    ...
    sheet_config = dict(st.secrets["google_sheet"])
...
def tracking_config() -> dict[str, str]:
    if "tracking" not in st.secrets:
        return {}
    config = dict(st.secrets["tracking"])
    return {
